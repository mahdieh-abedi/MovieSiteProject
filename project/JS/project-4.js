...
// movie entry
cardsInfo.push(new Card('../Img/AvengersEndgame.jpg', `ََAVENGER ENDGAME`, `8.8`, `SUN 8 SEPT - 10:00PM`, `2HR 05MIN`))
...
cardsInfo.push(new Card(`../Img//AquaMan.jpg`, `AQUA MAN`, `8.1`, `WED 16 AUG - 07:00PM`, `2HR 14MIN`))
...
function onSearch(element) {
    const searchTerm = element.value.toString().toUpperCase();
    const filteredCards = cardsInfo.filter(card => {
        return card.name.includes(searchTerm) || card.rate.includes(searchTerm) || card.rate.includes(searchTerm) || card.updateDate.includes(searchTerm)
    })
